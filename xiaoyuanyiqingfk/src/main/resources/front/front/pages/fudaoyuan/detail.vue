	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('fudaoyuan', this.fudaoyuanId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.fudaoyuanPhoto ? this.detail.fudaoyuanPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('fudaoyuanYuyue', {
									yonghuId: _this.user.id,
									fudaoyuanId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/fudaoyuanYuyue/list');
							}
						}
					});
				},
