	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('guanliliuy', this.guanliliuyId);
                    this.detail = res.data
					this.detail.guanliliuyContent = this.detail.guanliliuyContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.guanliliuyPhoto ? this.detail.guanliliuyPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('guanliliuyYuyue', {
									yonghuId: _this.user.id,
									guanliliuyId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/guanliliuyYuyue/list');
							}
						}
					});
				},
