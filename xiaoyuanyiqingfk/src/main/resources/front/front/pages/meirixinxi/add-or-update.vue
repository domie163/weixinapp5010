        ...
        <form class="app-update-pv">
            <view
                ...
                <input   disabled
                         ...
                         :disabled="ro.meirixinxiName" type="text" v-model="ruleForm.meirixinxiName" placeholder="信息标题"></input>
            </view>
                <view :style='{"boxShadow":"0","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"rgba(228, 232, 230, 1)","margin":"0 0 0px 0","borderWidth":"2rpx","borderStyle":"none none solid none ","height":"100rpx"}'
                      class="cu-form-group">
                    <view :style='{"width":"160rpx","fontSize":"28rpx","color":"var(--publicMainColor)","textAlign":"left"}'
                          class="title">信息类型</view>
                    <picker @change="meirixinxiTypesChange" :value="meirixinxiTypesIndex" :range="meirixinxiTypesOptions" range-key="indexName">
                        <view
                                :style='{"padding":"0 24rpx","boxShadow":"0 10rpx 0px rgba(0,0,0,0)","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"40rpx","borderWidth":"2rpx 2rpx 10rpx 2rpx","fontSize":"26rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
                                class="uni-input">{{ruleForm.meirixinxiTypes?ruleForm.meirixinxiValue:"请选择信息类型"}}</view>
                    </picker>
                </view>
                <view
                        ...
                        class="cu-form-group" @tap="meirixinxiPhotoTap">
                    <view :style='{"width":"160rpx","fontSize":"28rpx","color":"var(--publicMainColor)","textAlign":"left"}'
                          class="title">信息图片</view>
                    <view class="right-input" style="padding:0;textAlign:left">
                        <image
                                ...
                                class="avator" v-if="ruleForm.meirixinxiPhoto" :src="baseUrl+ruleForm.meirixinxiPhoto" mode="aspectFill">
                        </image>
            ...
            <view
                ...
                <textarea :style='{"padding":"0 24rpx","boxShadow":"0 10rpx 0px rgba(0,0,0,0)","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"40rpx","borderWidth":"2rpx 2rpx 10rpx 2rpx","fontSize":"26rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx","height":"200rpx"}'
                          :disabled="ro.meirixinxiContent" v-model="ruleForm.meirixinxiContent" placeholder="信息详情"/>
            </view>
...
<script>
    ...
    export default {
		...
		/*下拉框*/
			let meirixinxiTypesParams = {
                page: 1,
                limit: 100,
                dicCode: 'meirixinxi_types',
            }
			let meirixinxiTypes = await this.$api.page(`dictionary`, meirixinxiTypesParams);
			this.meirixinxiTypesOptions = meirixinxiTypes.data.list
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`meirixinxi`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.meirixinxiId){
                this.ruleForm.meirixinxiId = options.meirixinxiId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            ...
            async onSubmitTap() {
				if ((!this.ruleForm.meirixinxiName)) {
                    this.$utils.msg(`信息标题不能为空`);
                    return
                }
				if ((!this.ruleForm.meirixinxiPhoto)) {
                    this.$utils.msg(`信息图片不能为空`);
                    return
                }
                if (this.ruleForm.id) {
                    await this.$api.update(`meirixinxi`, this.ruleForm);
                } else {
                    await this.$api.save(`meirixinxi`, this.ruleForm);
                }
